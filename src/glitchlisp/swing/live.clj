...
(defn handle-live-line! [^JTextComponent editor-pane ^JLabel status line]
  (cond
    ...
    (when-let [[_ step] (re-matches #"STEP\s+([0-9]+)" line)]
      (swap! shared/state assoc :live-highlight-step (Long/parseLong step))
      (when (:live-awaiting-update @shared/state)
        (complete-live-update!)
        (shared/set-status! status "live running"))
      (editor/queue-live-step-highlight! editor-pane (Long/parseLong step)))
    ...
    (do
      (let [[_ tracks scenes] (re-matches #".*tracks=([0-9]+).*scenes=([0-9]+).*" line)]
        (swap! shared/state assoc
               :live-last-error nil
  ...
  (future
    (try
      ...
      (let [source (.getText editor-pane)
            _ (require-playback-form! source)
